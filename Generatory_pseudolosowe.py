...
from ploting import plot_numbers, plot_two_sets
import matplotlib.pyplot as plt


if __name__ == "__main__":
    add_lcg = Additive_LCG(47)
    mult_lcg = Multiplicative_LCG(1)
    gen = Generator(47, 53, 59)
    pm = Park_Miller(47)

    results_add_lcg = add_lcg.generate_n(3500, -3,3)
    results_mult_lcg = mult_lcg.generate_n(3500, -3,3)
    results_gen = gen.generate_n(3500,-3,3)
    results_pm = pm.generate_n(3500,-3,3)
    ...
    plot_numbers(results_gen, "Rozkład jednostajny -- MRG")
    plot_numbers(results_pm, "Rozkład jednostajny -- generator Park-Miler")
    plot_two_sets(results_gen, results_pm, "MRG vs. PM", "MRG", "PM")
    plt.ioff()
